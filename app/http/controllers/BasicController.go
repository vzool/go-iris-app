...
func BasicController(app *mvc.Application) {
	// You can use normal middlewares at MVC apps of course.
	app.Router.Use(func(ctx iris.Context) {
		ctx.Application().Logger().Infof("Path1: %s", ctx.Path())
		ctx.Next()
	})
...
// LoggerService If controller's fields (or even its functions) expecting an interface
...
func (c *basicController) Tut() string {
	return "Tutorial"
}
